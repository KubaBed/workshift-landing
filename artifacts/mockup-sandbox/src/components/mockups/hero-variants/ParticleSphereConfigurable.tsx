...
function createDot(pivot: THREE.Group, size: number, opacity: number, palette: THREE.Color[]): Dot {
...
export function ParticleSphereConfigurable({ config }: { config: SphereConfig }) {
  ...
  useEffect(() => {
    ...
    const init = (w: number, h: number) => {
      ...
      const lineOpacity = cfg.lineOpacity ?? 0.08;
      ...
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        const dot = createDot(pivot, size, opacity, palette);
        dots.push(dot);
        pivot.add(dot.mesh);
    ...
    return () => { ro?.disconnect(); cleanupFns.forEach(fn => fn()); };
  }, []);
