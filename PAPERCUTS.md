...
| 2026-08-30 | uslugi | `metaDescription` usługi `automatyzacja` w `src/data/services.js` brzmi „wdrożenie automatyzacji z n8n, Make i AI", a `expandedDescription` powtarza „budujemy pipeline: n8n, Make". To copy prowadzące, widoczne w SERP-ie, więc łamie regułę 4 z `BRAND.md` (w copy konsumenckim zero nazw stacku). Karta `innerCards[2]` typu `stack` wymienia te narzędzia **celowo** i ma zostać - reguła dotyczy leadu, nie sekcji technicznej niżej | keyword clustering 30.08, `docs/seo/PLAN-SEO-WDROZENIE.md` Sprint 1.1 | nie - rozjazd copy z regułą marki, adresowany w Sprincie 1 |
| 2026-08-30 | global | Przekierowanie apex na www ma dwa skoki, a drugi jest tymczasowy: `http://workshift.pl` → 308 → `https://workshift.pl/` → **307** → `https://www.workshift.pl/`. 307 to domyślne zachowanie Vercela; Google traktuje je jako tymczasowe, więc konsolidacja sygnałów na wariant www jest słabsza niż przy 301. Widać to w GSC: 3 zapytania raportowane na apex, 1 na www. Przy dzisiejszym zerowym profilu linkowym bez znaczenia | weryfikacja hosta 30.08 | nie - do poprawy zanim pojawią się linki zewnętrzne |
